    ...
    <div className="min-h-screen bg-[var(--bg-primary)] flex items-center justify-center p-6">
      ...
      <div className="w-full max-w-md relative z-10 animate-fade-in">
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-bold tracking-tight">Welcome <span className="gradient-text">back</span></h1>
          <p className="text-[var(--text-secondary)] mt-2">Sign in to your Vibe Coding workspace</p>
        </div>
        <div className="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-[var(--radius-xl)] p-8 shadow-xl shadow-black/20">
          <form action={async (formData) => { "use server"; await signIn("credentials", { email: formData.get("email"), password: formData.get("password"), redirectTo: "/" }); }}>
            <div className="space-y-4">
              ...
              <div>
                ...
                <input name="password" type="password" placeholder="••••••••" required className="w-full px-4 py-3 bg-[var(--bg-tertiary)] border border-[var(--border)] rounded-[var(--radius-md)] text-sm text-[var(--text-primary)] placeholder-[var(--text-tertiary)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/40 focus:border-[var(--accent)]/40 transition-all" />
              </div>
              <button type="submit" className="w-full py-3 bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-400 hover:to-purple-400 text-white font-medium rounded-[var(--radius-md)] transition-all shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 text-sm">Sign In</button>
            </div>
          </form>
        ...
        <p className="text-center text-sm text-[var(--text-tertiary)] mt-6">Don&apos;t have an account? <Link href="/register" className="text-[var(--accent)] hover:text-[var(--accent-hover)] font-medium">Create one</Link></p>
      </div>
    </div>
  );
}
